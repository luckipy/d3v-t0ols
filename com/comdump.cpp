...
static std::wstring GetTypeLibPath(const std::wstring& clsid)
{
    // 1. CLSID trực tiếp có TypeLib key
    wchar_t p[512];
    swprintf_s(p, L"CLSID\\%s\\TypeLib", clsid.c_str());
    std::wstring tlbid = RegReadString(HKEY_CLASSES_ROOT, p, L"");

    // 2. Nếu không có, tìm từ các CLSID cùng AppID (ví dụ Excel.Sheet.8 có TypeLib)
    if (tlbid.empty()) {
    ...
    if (!tlbid.empty()) {
        ...
        // Sort versions descending để lấy mới nhất
        std::sort(vers.rbegin(), vers.rend());
        for (auto& v : vers) {
            wchar_t vp[512];
            swprintf_s(vp, L"TypeLib\\%s\\%s\\0\\win64", tlbid.c_str(), v.c_str());
            std::wstring path64 = RegReadString(HKEY_CLASSES_ROOT, vp, L"");
            if (!path64.empty()) return ExpandEnv(path64);

            swprintf_s(vp, L"TypeLib\\%s\\%s\\0\\win32", tlbid.c_str(), v.c_str());
            std::wstring path32 = RegReadString(HKEY_CLASSES_ROOT, vp, L"");
            if (!path32.empty()) return ExpandEnv(path32);
        }
    }

    // 3. Fallback: InprocServer32 / LocalServer32
    ...
    return GetExecutablePath(ExpandEnv(RegReadString(HKEY_CLASSES_ROOT, p, L"")));
}
...
static std::wstring GetExecutablePath(const std::wstring& alreadyExpanded)
{
    ...
    if (s[0] == L'"') {
        ...
        return (end != std::wstring::npos) ? s.substr(1, end - 1) : s.substr(1);
    }

    // Walk token-by-token to find existing file
    size_t pos = 0;
    while (pos < s.size()) {
...
static TypeLibInfo LoadAndAnalyzeTypeLib(const std::wstring& path)
{
    ...
    std::wstring exe = GetExecutablePath(expanded); // GetExecutablePath không tự expand nữa
...
static DispatchProbe ProbeViaDispatch(const std::wstring& clsid)
{
    ...
    // 1. GetActiveObject (existing instance)
    HRESULT hr = GetActiveObject(cls, nullptr, (IUnknown**)&pDisp);

    // 2. CoCreateInstance với IDispatch
    if (FAILED(hr) || !pDisp)
        hr = CoCreateInstance(cls, nullptr, CLSCTX_ALL, IID_IDispatch, (void**)&pDisp);

    // 3. Nếu vẫn fail, thử qua IUnknown rồi QI sang IDispatch
    if (FAILED(hr) || !pDisp) {
    ...
    if (SUCCEEDED(hr) && pTI) {
        ...
        wchar_t msg[128];
        swprintf_s(msg, L"GetTypeInfo failed: 0x%08X", (unsigned)hr);
        result.errorMsg = msg;
    }
...
static void CheckImage(ServerInfo& si, const std::wstring& cmdLine)
{
    ...
    if (expanded[0] == L'"') {
        ...
        // Case 2: Walk tokens — chỉ accept khi là FILE (không phải directory)
        size_t pos = 0;
        while (pos < expanded.size()) {
            size_t sp = expanded.find(L' ', pos);
            std::wstring candidate = (sp == std::wstring::npos)
                ? expanded
                : expanded.substr(0, sp);
    ...
    if (!si.imgMissing) {
        ...
        HANDLE hf = CreateFileW(si.imagePath.c_str(), GENERIC_WRITE,
...
static void DumpDefaults()
{
    ...
    for (auto v : sv) {
        ...
        if (!val.empty()) {
            // Decode nếu là impersonation/auth level
            std::wstring display = val;
            std::wstring vname = v;
            std::transform(vname.begin(), vname.end(), vname.begin(), ::towlower);
            if (vname.find(L"impersonation") != std::wstring::npos)
                display = val + L"  (" + ImpLevelDecode(val) + L")";
            else if (vname.find(L"authentication") != std::wstring::npos && v != std::wstring(L"EnableDCOM"))
